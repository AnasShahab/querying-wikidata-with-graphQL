...
\lukas{maybe spell out RDF here and give citation or footnote to w3c standard}
...
\lukas{but what is a knowledge base? (I'm not sure what you want to say)}
...
Wikidata is an open knowledge graph developed by Wikimedia Deutschland. It contains structured data and acts as a central database for Wikimedia projects like Wikipedia.
\lukas{is wikidata indeed used for wikipedia? (I really do not know)}
The data in Wikidata is converted into RDF, and thus can be queried using SPARQL, which is a query language for RDF. However, in commercial applications the use of SPARQL remains limited. One of the main reasons is that developers are not often learned or experienced in the triples that RDF offers.
\lukas{I think part of the reason is also that sparql queries look more complex. Maybe you can give an example in the intro already that shows a query for both sparql and graphql to see the difference.}
...
\lukas{Use the same sentence structure for the bullet points; e.g. always start with: In chapter X, ...}
\begin{itemize}
	...
	\item Lastly, we provide a conclusion and future work in Chapter 5.
        \lukas{I think you do not have to mention the conclusion here}
\end{itemize}
...
\lukas{rather: At the end, we also compare SPARQL to GraphQL. or : At the end, SPARQL is also compared to GraphQL.; also: give details about what is to be compared exactly}
...
\lukas{is the idea of having classes with instances really part of RDF or only RDF Schema?.}
...
\lukas{I think the ``triples'' are also called statements. See also: https://www.w3.org/TR/rdf-primer/ and https://www.w3.org/TR/rdf11-primer/}
...
\lukas{I think I would leave out the first two paragraphs here. They are more suitable for the introduction and I think you mention some of this information there already.}
