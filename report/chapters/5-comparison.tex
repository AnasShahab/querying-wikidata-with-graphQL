...
\label{ch:5}
\lukas{didnt you want to rename this?}
...
\lukas{might be worthwhile to mention somewhere (not necessarily here) that this is not only possible via the command line but since this is just a javascript library, you can include it in frontend web applications, i.e. execute graphql queries agains wikidata directly from the without an intermediary server}

\lukas{another thing that crosses my mind and that I'm not sure where to put: GraphQL does not have a semantic on its own but the semantics are defined via the schema. This is also true for HypergraphQL to a large extent. However, GraphQL-LD does not have a schema but still defines semantics (included some arguments) for GraphQL (which we noticed are sometimes not so intuitive).}
...
\end{lstlisting}
\end{minipage}
\lukas{I'm still confused that there is no opening brace behind ``id(...)'' but I trust you on that :D}
...
\lukas{be consistent with amarican vs british english}
...
\begin{table}[h]
	...
	\end{center}
\end{table}
\lukas{make sure that the footnotes occur on the same page as the table}
