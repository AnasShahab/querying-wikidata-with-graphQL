...
Figure~\ref{fig:4} shows the RDF graph of our example represented in Figure~\ref{fig:2}. We use the \textit{http://www.w3.org/2000/01/rdf-schema\#label} property in RDF to provide a human-readable version of a resource's name.
...
Wikidata defines its own classes and properties that are equivalent to RDF constructs. For example, to denote that the subject is an instance of a class, Wikidata uses the "\textit{instance of}" property instead of the "\textit{rdf:type}" property in RDF. Wikidata offers an explanation for some of the standards properties in Wikidata that correspond to the ones in RDF \cite{Foundation}. 

Unlike RDF, statements in Wikidata are ranked. Additionally, they can have annotations and references. In Section~\ref{subsec:rdf-literals} we discussed that data types in RDF are mapped to XML Schema data types. However, property types in Wikidata are not defined in terms of XML Schema types. They are defined in terms of the types of values that the properties can have. For example, the "\textit{element symbol}" property in Wikidata(\footnote: https://www.wikidata.org/wiki/Property:P246) has the property type of "\textit{String}" indicating it can have values that are strings.

Resources in Wikidata are called entities, and each entity is uniquely identified by an \textit{entity ID}. The next section describes Wikidata entities and statements in detail.
...
Items are real-world objects, concepts or events.  These generally have a page in Wikipedia. For instance, there is an English Wikipedia on Helium - \textit{https://en.wikipedia.org/wiki/Helium}. Here the item is Helium and Wikidata records facts about it.\footnote{https://www.wikidata.org/wiki/Q560} Each item is identified by a \textit{QID} - an ID prefixed with the letter "\textit{Q}" followed by a number. For example, the item helium has the \textit{QID} of \texttt{Q560}. Items in Wikidata belong to the main namespace - \textit{http://www.wikidata.org/wiki/QID}. 
...
The select and where clauses are mandatory, the rest being optional. Other optional features are filters, groups, query operators such as UNION, OPTIONAL and BIND, and aggregates.\footnote{A full specification for the query language can be found on the official W3C documentation\cite{Seaborne}}
...
\lukas{I have no idea in how far Sparql might be used in comercial applications. Since some comercial applications definitely make use of Wikidata, I would assume they use Sparql as well. We should not make claims that we cannot back up by any citations. We know that GraphQL is popular in the web development community and that we think that it might make Wikidata more convenient to use but that's basically it. The idea is that this might lower the entry barrier for more people to actually integrate Wikidata into their applications but we do not know if it is just due to sparql or due to other reasons that people don't seem to do this to a large scale.}
...
\lukas{this is maybe too informal; I'm not sure if this is understood}
