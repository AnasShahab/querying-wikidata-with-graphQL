...
\begin{itemize}
	...
	\item the different concrete syntaxes such as Triples, which is shown in section XYZ \lukas{You can have labels for sections, chapters, etc. to be able to ref them}
\end{itemize}
...
GraphQL (Graph Query Language) is an open source query language for APIs (Application Programming Interfaces) and a runtime for executing those queries against existing data. It describes data structured in a graph format – a collection of objects (nodes) connected to each other by some kind of relationships (edges). Runtime means that the GraphQL API runs on the server side. \lukas{This sounds a little bit like that there is already a server-side in place when having GraphQL. I think in practice this is true but it is not generelly necessary. You could setup a standalone GraphQL Server connected to other services. Since you describe GraphQL Servers in the next paragraph, you can leave away this sentence I think.}
...
\lukas{An important part of the difference is that both query languages where developed with different goals in mind. Sparql is the natural query language for RDF graphs whereas GraphQL is designed to wrap REST APIs in a graph like shape to be able to have one query for information that would have required multiple REST API call before. (AFAIK) So GraphQL is not designed for what we use it here but we can argue that this idea is still natural since GraphQL Queries represent Graphs (i.e. trees) themselves.}
