...
According to RDFS, resources are divided into groups known as classes. Each member of a class is called an instance of that class, and is itself also a resource.
\lukas{do you mean that the instance is also a resource or the class is also a resource?}
The relationship between subject and object resources is described via RDF properties.
\lukas{sounds like subject and object resources have been introduced but I think they have not}
Essentially, the predicate of an RDF statement is an instance of RDF property. For instance, to identify that a resources is an instance of a class we use the predicate rdf:type which is in turn an instance of RDF property. W3cC provides a thorough documentation on RDF Schema.\footnote{w3.org/TR/2014/REC-rdf-schema-20140225/ .} In Figure 1 we see that Helium is an instance of the class chemical element. The property instance of describes the relation between the subject Helium and object chemical element.
\anas{Is the concept of property and predicate clear?}
\lukas{I think this needs some reworking still; you talk about a lot of things like subject, object, predicate, statements, ...; it sounds a bit like the reader should already know what some of these mean but I think you do not mention any of these ealier}
\lukas{also: are properties part of RDF or RDFS?, i.e. is the whole paragraph about RDFS or just the first sentence?}
...
\lukas{what exactly do you mean by building blocks here? are those the nodes or the edges or something else?}

\subsection*{IRI}\lukas{spell it out in title and maybe give abbreviation in parenthesis}
%\subsubsection{IRIs}
...
\lukas{maybe more detail how wikidata relates to RDF}
...
Queries in SPARQL are based on matching graph patterns and can be used to retrieve, add or delete data in the RDF based dataset. In section XYZ we saw
\lukas{avaid past tense}
that RDF data is based on triples - subject, object and predicate. Consequently, a query in SPARQL consists of a set of triple patterns. Each of the elements of the triple can be a variable (a string beginning with ? or \$) that needs to be queried. The solution to the variables is obtained by matching the query patterns to the triples in the dataset.
...
\begin{itemize}
\item Prologue: PREFIX and BASE keywords that function similarly to those in RDF turtle format\lukas{point there}
...
\lukas{I think it's confusing to say that graphql describes data. GraphQL is not a data store.}

GraphQL is usually served over HTTP through a GraphQL server.
\lukas{not sure with the wording; a graphql api might be served but graphql not really; first and foremost GraphQL is a query language}
A GraphQl server consists of two main parts – schema and resolver. The API developers create a schema that is strictly typed and describes all the possible data a client can query using the service. The schema specifies object types and fields along with operations on those types. The object type represents the kind of object that can be requested by a client. 
