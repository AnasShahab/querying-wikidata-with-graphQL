...
% \acrshort{URI}
\lukas{it's odd that you talk about IRIs but then you suddenly define a URI}
...
\lukas{you can try to end the paragraph on the line before (i.e. not only have a few symbols on the next line); this just looks nicer but it is not a real issue}
...
\begin{figure}[h]
  ...
  \label{fig:4}
\end{figure}
\lukas{figure might be hard to read}
...
%		\textbf{element} & \textbf{element_formula} & \textbf{element_label} & \textbf{boiling_point} & \textbf{melting_point} & \textbf{density} & \textbf{discoverer} & \textbf{place_birth} & \textbf{country} \\ \hline
	...
	\end{center}
\end{sidewaystable}
\lukas{table might be hard to read}
...
\end{lstlisting}
\end{minipage}
\lukas{did you explain what ``[ChemicalElement]'' means?}
...
\end{lstlisting}
\end{minipage}
\lukas{I think i GraphQL to be able to use limit or other arguments, they need to be defined in the schema (which is not the case here)}
...
\end{lstlisting}
\end{minipage}
\lukas{Again: I think i GraphQL to be able to use first, checmicalFormula or other arguments, they need to be defined in the schema (which is not the case here)}
