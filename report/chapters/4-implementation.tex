...
\lukas{give the solutions names that reflect their key idea}
...
\lukas{what was the benefit of solution 2 over 1 again?}
...
The above three proposed solution are prospective workarounds for describing the "instance of" relationship between two items in Wikidata. Solutions 1 and 2 are more feasible then Solution 3. This way of describing the "instance of" relationship using inline fragments is not intuitive, as they are primarily used to query a field that returns an interface or a union type.\footnote{https://graphql.org/learn/queries/\#inline-fragments.}
...
HyperGraphQL translates the root node to be an rdf:type of some subject. Since this does not comply with the Wikidata data model, we had a similar issue like the one we discussed in Solution 3 of the previous section regarding inline fragments in GraphQL-LD. To overcome this issue we had to update the source code so that the generated SAPRQL queries use instance of property instead of rdf:type.
