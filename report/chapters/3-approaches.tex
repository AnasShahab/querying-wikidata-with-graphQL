...
\lukas{just a reminder: be careful to keep a balanced comparison; SPARQL has obvious advantages over GRAPHQL in being (presumably) more expressive, e.g. because of basic graph patterns}
...
\anas{How can I explain academic approach?}
...
\end{lstlisting}
\end{minipage}
\lukas{reminder: same spacing on left and right would look better!}
...
\end{itemize}
\lukas{maybe give like a one sentence description for each at this point}


\subsection{GraphQL to SPARQL algebra}

The "GraphQL to SPARQL algebra" module is used for parsing a GraphQL query into SPARQL algebra\footnote{https://www.w3.org/TR/sparql11-query/\#sparqlQuery.} with the help of a JSON-LD context. SPARQL algebra \lukas{being picky here but SPARQL algebra itself is probably not an expression; you could say ``A SPARQL algebra expression is formed by ...''}is an expression formed from parsing the strings in SPARQL query followed by some transformations. It is basically used to provide semantics to the syntax in SPARQL query.

The algorithm for the conversion of GraphQL query to SAPRQL algebra is based on translating the tree-like structure of GraphQL to links of triple patterns or statements  in SPARQL \cite{Taelman2018}. Listing XYZ shows the SPARQL algebra \lukas{the SPARQL algebra expression (?)}obtained by parsing the GraphQL query in Listing XYZ.
...
\lukas{hope I told you that already: you can use ``label'' and ``ref'' to get section numbers or listing numbers or almost anything else}
...
\lukas{I think for DBPedia, which is what GraphQL-LD is designed for, this was not a problem since each entity also had a property with the same name(?); maybe mention this (if it is true)}
